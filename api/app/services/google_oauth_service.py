...
from app.services.user_mgmt_service import update_user_credentials_service, verify_user_email_service, verify_user_id_service
...
from app.utils.auth_utils import calculate_token_expiry, construct_oauth_url, generate_jwt_response, get_valid_refresh_token, handle_token_exchange_errors, is_token_expired
...
async def handle_google_auth(auth_request: AuthRequestDto, db: Session):
    ...
    user = verify_user_email_service(email, db)
    if not user:
...
async def handle_token_refresh(user_id: str, db: Session):
    ...
    user = verify_user_id_service(user_id, db)
